 ...
 * 百度主动推送
 */
if (io_get_option('baidu_submit',false,'switcher')) {
    // 主动推送
    if(!function_exists('Baidu_Submit')){
		function Baidu_Submit($post_ID) {
			...
			if (array_key_exists('success',$result)) {
				 ...
				 * -----------------------------------------------------------------------
				 * HOOK : ACTION HOOK
				 * io_baidu_push_success
				 * 
				 * 百度主动推送成功后执行。
				 * @since  3.1418
				 * -----------------------------------------------------------------------
				 */
				do_action( 'io_baidu_push_success' ,$post_ID,$result );
			}
		}
 ...
 * 熊掌号自动推送
 */
if ( io_get_option('baidu_xzh',false,'switcher')) {
	if(!function_exists('Baidu_XZH_Submit')){
		function Baidu_XZH_Submit($post_ID) {
			...
			if (array_key_exists('success_realtime',$result)) {
				 ...
				 * -----------------------------------------------------------------------
				 * HOOK : ACTION HOOK
				 * io_bear_paw_push_success
				 * 
				 * 熊掌号自动推送成功后执行。
				 * @since  3.1418
				 * -----------------------------------------------------------------------
				 */
				do_action( 'io_bear_paw_push_success' ,$post_ID,$result );
			}
		}
...
function io_baidu_excerpt($len=220){
	if ( is_single() || is_page() ){
		...
		if ($post->post_excerpt) {
			...
			if(preg_match('/<p>(.*)<\/p>/iU',trim(strip_tags(do_shortcode($post->post_content),"<p>")),$result)){
				$post_content = $result['1'];
			} else {
				$post_content_r = explode("\n",trim(strip_tags(do_shortcode($post->post_content))));
