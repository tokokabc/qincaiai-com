...
class ThemeUpdateChecker {
	 ...
	 * @param string $maxVersion 子主题支持的最大版本
	 * @param boolean $enableAutomaticChecking 启用/禁用自动更新检查。如果设置为FALSE，则需要主动调用checkForUpdates()来检查更新。
	 */
	public function __construct($theme, $metadataUrl, $maxVersion = '', $enableAutomaticChecking = true){
		$this->theme = $theme;
	...
	public function checkForUpdates(){
		...
		if ( empty($state) || time() - $state->lastCheck > 300 ){ //缓存5分钟
			$state = new StdClass;
			...
			update_option($this->optionName, $state, false); //检查前保存以防出错
			
			$state->update = $this->requestUpdate();
			update_option($this->optionName, $state, false);
		}
		return $state;
	}
	...
	public function onTransientUpdate($value){
		...
		&& (empty($this->maxVersion) || version_compare( $this->maxVersion, $state->update->toWpFormat()['new_version'], '>=' ))
		) {
			...
			$item = array(
				...
				'url'          => 'https://www.iotheme.cn/store/'.$this->theme.'.html',
				'package'      => 'https://www.iotheme.cn/store/'.$this->theme.'.html#update-log',
				'requires'     => 5.6,
				'requires_php' => 7.2,
			);
...
class ThemeUpdate {
	 ...
	 * @return ThemeUpdate|null ThemeUpdate 的新实例，错误时为NULL。
	 */
