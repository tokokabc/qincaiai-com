 ...
 *
 */
if ( ! class_exists( 'IOCF_Field_typography' ) ) {
  class IOCF_Field_typography extends IOCF_Fields {
      ...
      echo '<div class="csf--typography'. esc_attr( $chosen_class ) .'" data-depend-id="'. esc_attr( $this->field['id'] ) .'" data-unit="'. esc_attr( $args['unit'] ) .'" data-line-height-unit="'. esc_attr( $line_height_unit ) .'" data-exclude="'. esc_attr( $args['exclude'] ) .'">';
        ...
        echo '<div class="csf--blocks csf--blocks-selects">';
          ...
          if ( ! empty( $args['backup_font_family'] ) ) {
            ...
            echo $this->create_select( apply_filters( 'iocf_field_typography_backup_font_family', array(
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        IOCF::include_plugin_file( 'fields/typography/google-fonts.php' );

        wp_enqueue_script( 'csf-webfontloader', 'https://cdn2.iocdn.cc/npm/webfontloader@1.6.28/webfontloader.min.js', array( 'csf' ), '1.6.28', true );

        $webfonts = array();

        $customwebfonts = apply_filters( 'iocf_field_typography_customwebfonts', array() );
        ...
        $webfonts['safe'] = array(
          ...
          'fonts' => apply_filters( 'iocf_field_typography_safewebfonts', array(
            'Arial',
            'Arial Black',
            'Helvetica',
            'Times New Roman',
        ...
        $webfonts['google'] = array(
          ...
          'fonts' => apply_filters( 'iocf_field_typography_googlewebfonts', iocf_get_google_fonts()
        ) );

        $defaultstyles = apply_filters( 'iocf_field_typography_defaultstyles', array( 'normal', 'italic', '700', '700italic' ) );

        $googlestyles = apply_filters( 'iocf_field_typography_googlestyles', array(
          '100'       => 'Thin 100',
        ...
        $webfonts = apply_filters( 'iocf_field_typography_webfonts', $webfonts );

        wp_localize_script( 'csf', 'iocf_typography_json', array(
          'webfonts'      => $webfonts,
    ...
    public function enqueue_google_fonts( $method = 'enqueue' ) {
      ...
      if ( ! empty( $this->value['type'] ) ) {
        ...
        IOCF::include_plugin_file( 'fields/typography/google-fonts.php' );
        $is_google = ( array_key_exists( $this->value['font-family'], iocf_get_google_fonts() ) ) ? true : false;
      }
      ...
      if ( $is_google ) {
        ...
        if ( $font_weight || $font_style ) {
          ...
          if ( ! empty( $style ) ) {
            ...
            IOCF::$webfonts[$method][$font_family][$style] = $style;
          }
        } else {
          IOCF::$webfonts[$method][$font_family] = array();
        }
        ...
        if ( ! empty( $this->value['extra-styles'] ) ) {
          foreach ( $this->value['extra-styles'] as $extra_style ) {
            if ( ! empty( $extra_style ) ) {
              ...
              IOCF::$webfonts[$method][$font_family][$extra_style] = $extra_style;
            }
          }
        }
