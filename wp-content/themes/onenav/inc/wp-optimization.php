...
function io_filter_register_post_type_args($args, $post_type)
{
    ...
    if (did_action('init') && !empty($args['supports']) && is_array($args['supports']) && in_array($post_type, ['post', 'page'])) {
        foreach (['trackbacks' => 'disable_trackbacks', 'revisions' => 'disable_revision'] as $support => $setting_name) {
            //echo $setting_name;
            if (io_get_option($setting_name, false) && is_array($args['supports']) && in_array($support, $args['supports'])) {
                // 遍历数组，找到并移除匹配的 support
                $support_removed = false;
                foreach ($args['supports'] as $key => $value) {
                    if ((is_string($value) && $value === $support) || $key === $support) {
...
add_filter('pre_get_avatar_data', function($args, $id_or_email){
    ...
    switch ($gravatar_cdn){
        ...
        case "chinayes":
            ...
            break;
        case "iocdn":
            $url    = '//cdn2.iocdn.cc/avatar/'.$email_hash;
            break;
        case "qiniu":
