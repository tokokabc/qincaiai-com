...
if ( ! defined( 'ABSPATH' ) ) { exit; }  ?>
        <div class="card hot-card mb-0 br-xl">
            ...
            <div class="card-body pb-3 pt-0">
                <div class="overflow-auto hot-body">
                    <ul id="hot_news-<?php echo $t ?>">
                    </ul>
                </div>
                <div class="d-flex text-xs text-muted pt-2 mb-n2">
                    <a href= "<?php echo esc_url(home_url().'/hotnews/') ?>" title='<?php _e('更多','i_theme') ?>' style="color:#6c757d; <?php echo ((is_home() || is_front_page() || is_mininav())?'':'display:none') ?>" ><?php _e('更多','i_theme') ?></a>
                    <div class="flex-fill"></div>
                    <span><a href= "javascript:" id="hot-lod-<?php echo $t ?>" title='<?php _e('刷新','i_theme') ?>' style="color:#6c757d" ><i class="iconfont icon-refresh text-md"></i></a></span>
                </div>
            </div>
        </div>
        <div id="hot-loading-<?php echo $t ?>" class="ajax-loading text-center rounded" style="position:absolute;display:flex;width:100%;left:0;top:0;bottom:0;background:rgba(125,125,125,.5)"><div id="hot-success-<?php echo $t ?>" class="col align-self-center"><i class="iconfont icon-loading icon-spin text-32"></i></div></div>
<script>
(function($){ 
    ...
    $('#hot-lod-<?php echo $t ?>').on('click', function() {
        ...
        $("#hot-success-<?php echo $t ?>").html('<i class="iconfont icon-loading icon-spin text-32"></i>');
        $("#hot-loading-<?php echo $t ?>").fadeIn(200); 
        getList(ruleId);
    });
    function getList(id){
        $.get("<?php echo $api ?>", { rule_id: id,key:"<?php echo $key ?>" },function(response,status){ 
            var html = '';
